"""企业级 RAG + Agent + API（最终体：单文件版 666666.py）
...
4) 启动服务：python zhi_liao_knowledge_assistant/666666.py
    ...
    def decorator(func: callable) -> callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                REQUEST_LATENCY.labels(endpoint=endpoint, method=func.__name__).observe(time.time() - start)
                REQUEST_COUNTER.labels(endpoint=endpoint, method=func.__name__, status=status).inc()
...
class RAGAgent:
    ...
    def chat(self, req: AgentRequest) -> Dict[str, Any]:
        ...
        self.rag.set_current_user(req.user)
        ...
        if intent == "search":
            docs, scores = self.rag.search_documents(req.message, k=5, department=req.department, user_roles=req.user_roles)
            results = []
        ...
        # rag_answer
        docs, _ = self.rag.search_documents(req.message, k=5, department=req.department, user_roles=req.user_roles)
        if not docs:
...
def create_app() -> Any:
    ...
    def search(req: SearchRequest):
        ...
        try:
            rag.set_current_user(req.user)
            docs, scores = rag.search_documents(req.query, k=req.k, department=req.department, user_roles=req.user_roles, freshness_weight=req.freshness_weight)
            results = [
                {
                    "ref": i + 1,
    ...
    def query(req: QueryRequest):
        ...
        try:
            rag.set_current_user(req.user)
            docs, _ = rag.search_documents(req.question, k=5, department=req.department, user_roles=req.user_roles)
            if not docs:
...
if __name__ == "__main__":
    # 启动：python zhi_liao_knowledge_assistant/666666.py
