"""企业级 RAG + Agent + API（最终版：DeepSeek LLM + 本地 BGE Embedding）

说明：此版本用于兼容新版依赖栈，并采用"本地 Embedding + DeepSeek LLM"的最佳实践：
- Embedding: 本地 BAAI/bge-large-zh-v1.5，保证 ingest/search 稳定不依赖外网
- LLM: DeepSeek API (通过 OpenAI 兼容接口)，保证生成效果
- Pydantic v2 + LangChain 新版拆包
...
快速开始：
1) 建环境（推荐）
   conda create -n rag_demo_v2 python=3.10 -y
   conda activate rag_demo_v2
2) 安装依赖：pip install -r zhi_liao_knowledge_assistant/requirements_v2.txt
3) 配置 .env（项目根目录）：
   AUTH_MODE=jwt
   JWT_SECRET=...（长随机字符串）
   LLM_PROVIDER=openai
...
4) 准备文档：把 pdf/docx/txt 放到 ./zhi_liao_knowledge_assistant/demo_docs
5) 启动服务：python 最终版3_deepseek.py
...
# OpenAI embeddings / chat model（新版建议从 langchain_openai 导入）
try:
...
class RAGConfig(BaseSettings):
    ...
    # Embedding/LLM
    use_openai: bool = False  # 默认用本地 embedding，避免依赖 OpenAI
    openai_api_key: Optional[str] = None
    openai_chat_model: str = "deepseek-chat"  # 使用 DeepSeek API
    # 兼容 OpenAI-compatible 网关（如 DeepSeek 等），对应环境变量：OPENAI_BASE_URL
    openai_base_url: Optional[str] = "https://api.deepseek.com/v1"  # 默认使用 DeepSeek API

    llm_provider: str = "openai"  # openai | huggingface

    embedding_model: str = "BAAI/bge-large-zh-v1.5"  # 使用 bge-large-zh，性能更强
    ...
    model_config = {
        ...
        # 允许 .env 里出现一些多余字段，不至于启动直接挂（面试演示更稳）
        "extra": "ignore",
    }
    ...
    def build_or_load(self, documents: Optional[List[Document]] = None) -> None:
        try:
            ...
            if self.config.vectorstore_type == "faiss":
                if FAISS is None:
                    raise VectorStoreError("未安装/不可用 FAISS，请改用 chroma 或安装 faiss")
                if documents:
                    self.vectorstore = FAISS.from_documents(documents, self.embeddings)
                    self.vectorstore.save_local(self.config.vectorstore_path)
                else:
                    self.vectorstore = FAISS.load_local(self.config.vectorstore_path, self.embeddings, allow_dangerous_deserialization=True)
                logger.info("✓ 向量数据库已就绪 (类型: faiss)")
                return
...
class EnterpriseRAGSystem:
    def __init__(self, config: Optional[RAGConfig] = None) -> None:
        ...
        # 兼容：很多人习惯配置 OPENAI_API_KEY；如果 openai_api_key 未设置但环境变量存在，则注入
        if not self.config.openai_api_key:
            env_key = os.environ.get("OPENAI_API_KEY")
            if env_key:
                self.config.openai_api_key = env_key

        # 兼容：base_url（如果未设置，使用默认的 DeepSeek API）
        if not self.config.openai_base_url:
            env_base = os.environ.get("OPENAI_BASE_URL")
            if env_base:
                self.config.openai_base_url = env_base
            else:
                # 如果没有环境变量，使用默认的 DeepSeek API
                self.config.openai_base_url = "https://api.deepseek.com/v1"
    ...
    def _init_embeddings(self) -> Any:
        ...
        if self.config.use_openai and self.config.openai_api_key:
            if OpenAIEmbeddings is None:
                raise ImportError("缺少依赖 langchain-openai，请安装：pip install langchain-openai")
            os.environ["OPENAI_API_KEY"] = self.config.openai_api_key
            logger.info("使用 OpenAI Embeddings: text-embedding-3-small")
            if self.config.openai_base_url:
                return OpenAIEmbeddings(model="text-embedding-3-small", base_url=self.config.openai_base_url)
            return OpenAIEmbeddings(model="text-embedding-3-small")

        # HF Embeddings - 使用 ModelScope（国内可访问）
        # 设置 HF_ENDPOINT 环境变量，使 HuggingFace 从 ModelScope 下载模型
        if "HF_ENDPOINT" not in os.environ:
            os.environ["HF_ENDPOINT"] = "https://www.modelscope.cn"
        logger.info(f"使用 HuggingFace Embeddings (ModelScope): {self.config.embedding_model}")
    ...
    def _save_metadata_db(self) -> None:
        ...
        with open(fpath, "w", encoding="utf-8") as f:
            json.dump({k: v.model_dump() for k, v in self.metadata_db.items()}, f, default=str, indent=2, ensure_ascii=False)
    ...
    def init_llm(self, force_reload: bool = False) -> None:
        ...
        if provider == "openai":
            ...
            os.environ["OPENAI_API_KEY"] = self.config.openai_api_key
            if self.config.openai_base_url:
                self.llm = ChatOpenAI(model=self.config.openai_chat_model, temperature=0.1, base_url=self.config.openai_base_url)
            else:
                self.llm = ChatOpenAI(model=self.config.openai_chat_model, temperature=0.1)
            return
...
class RAGAgent:
    ...
    def chat(self, req: AgentRequest) -> Dict[str, Any]:
        ...
        user_roles = getattr(req, "user_roles", ["public"])
...
# 修复 Pydantic v2 / OpenAPI：不要在 create_app 内定义请求模型
...
def create_app() -> Any:
    ...
    def agent_chat(req: AgentRequest, ident: Identity = Depends(_dep_identity)):
        req.user = ident.user  # type: ignore[attr-defined]
        req.user_roles = ident.roles  # type: ignore[attr-defined]
